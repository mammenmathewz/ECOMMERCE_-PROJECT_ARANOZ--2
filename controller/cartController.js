...
const getCart = async (req, res, next) => {
  try {
    ...
    if (cart.discount > 0) {
      ...
      if (
        now - cart.discountAppliedAt > discountDuration ||
        cart.items.length == 0
      ) {
        cart.discount = 0;
  ...
  try {
    ...
    if (coupon && cart.total < coupon.min_amount) {
      return res
        .status(400)
        .json({
          message: `The minimum order value for this coupon is ${coupon.min_amount}. So remove the coupon to remove product.`,
        });
    }
    ...
    res.json({
      ...
      items: cart.items,
    }); // Send back the updated total, grand total and items
  } catch (error) {
    console.log(error);
    next(error);
  }
  ...
  try {
    ...
    if (coupon && cart.total < coupon.min_amount) {
      return res
        .status(400)
        .json({
          message: `The minimum order value for this coupon is ${coupon.min_amount}.So remove the coupon to decrement quantity`,
        });
    }
  ...
  try {
    ...
    if (cart.total < coupon.min_amount) {
      return res
        .status(400)
        .json({
          message: `The minimum order value for this coupon is ${coupon.min_amount}. The cart total is less than that.`,
        });
    }
    ...
    if (coupon.users.includes(userId)) {
      return res
        .status(400)
        .json({ message: "User has already used this coupon" });
    }
    ...
    if (coupon.discount_type === "amount") {
      discountAmount =
        Math.floor(
          Math.random() * (coupon.max_discount - coupon.discount_value + 1)
